    ...
    function image($type = 0, $length = 4, $lefttime = 900, $options = null)
    {
        if ($this->keepCode && $this->_code != '') {
            ...
            case 0:
                ...
                break;
            case 1:
                $seed = '346789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
                break;
            default:
      ...
      //  $color = isset($options['color']) ? $options['color'] : '0xffffff';
		$color = isset($options['color']) ? $options['color'] : '0x000000';
        //$bgcolor = isset($options['bgcolor']) ? $options['bgcolor'] : '0x666666';
		$bgcolor = isset($options['bgcolor']) ? $options['bgcolor'] : '0xffffff';
		
        $border = isset($options['border']) ? (int)$options['border'] : 0;
       // $bdColor = isset($options['borderColor']) ? $options['borderColor'] : '0x000000';
		 $bdColor = isset($options['borderColor']) ? $options['borderColor'] : '#ffffff';
		...
		$arr_code = str_split($code);
		$stp = '';
		foreach($arr_code as $cd)
		{
			$color = '0x'.rand(111111, 999999);
			list($r, $g, $b) = $this->_hex2rgb($color);
			$textColor = imagecolorallocate($img, $r, $g, $b);
			imagestring($img, $font, $paddingLeft  + $stp, $paddingTop + $border, $cd, $textColor);
			$stp += 10;
		}
        // 输出图像
        ob_clean();
