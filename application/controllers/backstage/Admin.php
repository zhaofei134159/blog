...
class Admin extends Admin_Controller {
	...
	public function index(){
        ...
        $count = $this->zf_admin_model->count($where);

        list($offset, $page_htm) = $this->pager->pagestring($count, $pagesize);

		$admins = $this->zf_admin_model->get_list($where,'*','ctime desc',$pagesize, $offset);
		...
		$this->load->view(ADMIN_URL.'admin/index',$data);
	}
	...
	{	
		...
		$admin = array();
		if(!empty($id)){
			$admin = $this->zf_admin_model->select_one('id='.$id);
		}
		...
		$this->load->view(ADMIN_URL.'admin/edit',$data);
	}
	...
	public function update(){
		...
		if(isset($post['id'])&&!empty($post['id'])){
			...
			$this->zf_admin_model->update($post,'id='.$id);
		}else{
			$this->zf_admin_model->insert($post);
		}
	...
	public function check_phone(){
		...
		$flog = 0;
		$msg = '不存在';
		$admin = $this->zf_admin_model->select_one('phone='.$post['phone']);
		if(!empty($admin)){
			$flog = 1;
			$msg = '存在';
		}
	...
	public function state(){
		...
		$admin = $this->zf_admin_model->select_one('id='.$post['id']);
		if($admin['is_del']==$is_del){
			$is_del = 1;
		}

		$this->zf_admin_model->update(array('is_del'=>$is_del),'id='.$post['id']);
